...
class WeatherCard extends Control {
  constructor(parentNode) {
    ...
    this.chl = [];
    for (let i = 0; i < 7; i++) {
      const slide = this.slider.addSlide('');
  ...
  refresh(dataList, backBuffer, options) {
    ...
    const { unit } = options;
    const lang = options.langIndex;

    const feelsPref = ['feels like', 'ощущается', 'адчуваецца'][lang];
    const windPref = ['wind speed', 'скорость ветра', 'хуткасць ветру'][lang];
    const humidityPref = ['humidity', 'влажность', 'вiльготнасць'][lang];
...
class WeatherSubCard extends Control {
  ...
  refresh(data, unit, options) {
    console.log(data);
    const date = new Date(Date.parse(data.observation_time.value));
