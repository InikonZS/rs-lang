...
class Statistic extends Control{
  constructor(parentNode, base){
    ...
    let makeSorted = function(comparator){
      let baseWords = that.base.words.sort(comparator);
      
      let tr = new Control(that.node, 'tr');
      let bf;
  ...
  drawSorted(comparator){
    let baseWords = this.base.words.sort(comparator);
